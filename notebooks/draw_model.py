    ...
    def start(self, epoch_num, phase):
        ...
        for epoch in range(epoch_num):
            for i in range(int(len(dataloader.data)/self.batch_size)):
                ...
                data = np.reshape(data, (data.shape[0], 1, self.A, self.B))
                ...
                if count % a == 0:
                    ...
                    if count % b == 0:
                        if phase == 'train':
                            torch.save(self.state_dict(),'save/weights_%d.tar'%(count))
                        generate_image(count,self.T,self.A,self.B,self)
                    avg_loss = 0
        if phase == 'train':
...
def provider(path, phase):
    # TODO: change generation for one image
...
if __name__ == '__main__':
    ...
    path = os.path.join(root_path, config['DRAW']['path']) + "/cat.npy"
    ...
    epoch_num = 1 #int(config['DRAW']['epoch_num'])
