...
def xrecons_grid(batch_size,B,A,T,count,model):
    """
...
def save_image(x,count, T, A, B, model):
    for t in range(T):
        img = xrecons_grid(x[t],B,A,T,count,model)
        plt.matshow(img, cmap=plt.cm.gray)
        #plt.show()
        # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
        imgname = 'image/count_%d_%s_%d.png' % (count,'test', t)
...
def generate_image(count,T,A,B,model):
    x = model.generate()
